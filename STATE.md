...
after a real Sub-rendered frame, it begins a compact `FrameUploadPump`, plans
two 235-tile queues from pump-owned state, sends both through
`FB_UpdateTileQueue()`, reaches phase `0x87ff`, and verifies the second slice
restores a poisoned VRAM word to the Word RAM value `0xf11f`.
`src/main/frame_upload_pump.c` now adds the host-tested callback loop policy
seam that will own this cursor in the live path: it advances one budgeted queue
per tick, does not allow a new frame while one is uploading or waiting for Word
RAM return, and rewinds the cursor on upload failure so a failed tick is not
...
`0x2a06`. The pump proof now also passes a frame number through
`CMD_RENDER_FRAME` parameter 0, has the Sub render it as `Frame 4` with the
real SGDK-derived font on the fourth cycle, and captures the final visible
frame through BlastEm's internal screenshot path at
...
| Desktop full pump upload probe | Passing | `DESKTOP_PUMP_PROBE=1` + `-Probe DesktopPump` proves four compact-pump render/upload/return cycles; terminal phase `0x88ff`, pump result `0x0001`, frame count/status word `0x0004`, per-frame slice count `0x0005`, final span `0x03ac/0x00b4`, MEM_MODE `0x2a06`, final status `0x0003`, trace `0x7404`; debugger-backed screenshot captures the fourth frame marker at `C:\tmp\segaos_screens_internal\segaos_pump_frame_20260703_172624.png` |
...
- Boot-safe scheduler upload probe usage: Main IP 3,568 bytes / SP 11,836
  text bytes observed locally with `DESKTOP_SCHEDULER_PROBE=1`; this diagnostic
  Main build uses a compact `FrameUploadPump` planner, `-Os`, and skips mouse
  init plus framebuffer tilemap/palette setup to stay under the 3,584-byte IP
...
- Boot-safe full pump upload probe usage: Main IP 3,516 bytes / SP 11,944 text
  bytes observed locally with `DESKTOP_PUMP_PROBE=1`; this size-fit diagnostic
  uses a pump-local minimal VDP init plus active Plane A/palette setup, proves
  the compact planner can upload all five frame slices for four complete
  render/upload/return cycles inside the BIOS-loaded IP window, and supports
  the debugger-backed `Frame 4` visual capture.
...
- Target scheduler bridge: `DESKTOP_SCHEDULER_PROBE=1` proves the first two
  235-tile compact-pump planner slices can feed successive
  `FB_UpdateTileQueue()` calls on the live Main VDP path
- Frame upload pump: a full 1,120-tile frame under the 7,524-byte NTSC budget
  reaches Word RAM return readiness after five callback ticks: four 235-tile
  uploads plus one final 180-tile upload; the inline compact planner covers the
...
- Target pump bridge: `DESKTOP_PUMP_PROBE=1` proves the compact planner can
  ...
  after the final slice, and repeat that policy for four complete frames. This
  harness now uses a pump-local minimal VDP init plus active Plane A/palette
  setup so the same proof can be captured visibly without overflowing the
...
- `DESKTOP_PUMP_PROBE=1` proves the next frame-policy step: the compact
  ...
  render/upload/return frames. It now also initializes enough VDP display state
  for a visible fourth-frame marker capture without taking the full default
  `VDP_Init()` size cost.
...
| Frame Upload Pump | Main/host | `include/frame_upload_pump.h`, `src/main/frame_upload_pump.c` | Host-tested compact planner plus callback state machine that advances one scheduled upload per tick and gates Word RAM return until upload completion |
