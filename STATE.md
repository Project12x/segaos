...
| Host tests | Passing | `make host-tests` covers dirty-rect clipping, half-open intersection, root/window redraw planning, subtraction strips, edge-touch merge, corner-touch separation, overflow collapse, 8x8 tile range mapping, dirty tile transfer budgeting, dirty tile upload queue planning, BASIC program-buffer parsing/token storage/replacement/deletion/decoding plus shell line entry/LIST/NEW/RUN, integer/string expression evaluation, sequential PRINT/END execution, GOTO target resolution and step-limit handling, A-Z integer `LET` variables and runtime expression lookup, framebuffer tile-span conversion, dirty-queue upload chunking, storage save-target policy, and the fake-GDB timeout regression for the BlastEm probe harness |
...
- BASIC program-buffer/shell seam: `BAS_StoreSourceLine()` and
  ...
  lookup. This is not an `INPUT`, `IF`/`THEN`, string-variable, array, desktop
  I/O, or persistence layer yet.
