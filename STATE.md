...
| Host tests | Passing | `make host-tests` covers dirty-rect clipping, half-open intersection, root/window redraw planning, subtraction strips, edge-touch merge, corner-touch separation, overflow collapse, 8x8 tile range mapping, dirty tile transfer budgeting, dirty tile upload queue planning, BASIC program-buffer parsing/token storage/replacement/deletion/decoding plus shell line entry/LIST/NEW/RUN, integer/string expression evaluation, sequential PRINT/END execution, GOTO target resolution and step-limit handling, framebuffer tile-span conversion, dirty-queue upload chunking, storage save-target policy, and the fake-GDB timeout regression for the BlastEm probe harness |
...
- Storage policy seam: `STG_PlanSave()` is host-tested. It prefers external
  ...
  still pending.
- BASIC program-buffer/shell seam: `BAS_StoreSourceLine()` and
  `BAS_SubmitConsoleLine()` are host-tested as clean-room fixed-storage BASIC
  ...
  storage, decode stored lines, list programs through a caller-supplied sink,
  clear with `NEW`, and evaluate simple signed 16-bit integer `+`/`-`
  ...
  expression lines, routes output through the caller-supplied sink, and supports
  literal-line `GOTO` with missing-target and step-limit errors. This is not a
  variables, `INPUT`, `IF`/`THEN`, desktop I/O, or persistence layer yet.
...
| BASIC Core | Sub/host | `src/sub/basic.c` | Clean-room fixed-storage BASIC program buffer and shell/evaluator/runner seam with numbered-line parsing, keyword tokenization, sorted insert/replace/delete, compaction, decode, line entry, `LIST`, `NEW`, simple integer/string values, sequential `PRINT`/`END`, and literal-line `GOTO` |
